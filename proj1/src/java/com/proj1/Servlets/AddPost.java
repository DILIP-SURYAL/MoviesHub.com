...
import com.proj1.Databases.Post_database;
...
public class AddPost extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Post p = new Post(pTitle, pContent, pCode, null, part.getSubmittedFileName(), cid, user.getId());
            
            Post_database post = new Post_database(ConnectionProvider.getConnection());
            if(post.savePost(p)){
              
                
                String path = "D:\\java_programs\\proj1\\build\\web\\movie_picture\\" + part.getSubmittedFileName();
                Helpers.saveImage(part.getInputStream(), path);
